...
export class AccessMiddleware {
  ...
  async authorizeRequest(token: string, requestedOrgId: string): Promise<AccessDecision> {
    ...
    // Starter bug: runtime cache wins over DB revocation and membership drift.
    if (runtime?.status === "active" && new Date(runtime.expiresAt) > new Date() && runtime.orgId === requestedOrgId) {
      return { allow: true, reason: "runtime_session_active", session: runtime };
    }
    ...
    const active = dbSessions.find((session) => session.status === "active");
    if (!active) return { allow: false, reason: "db_session_missing" };
